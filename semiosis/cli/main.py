...
from semiosis.cli.factories import (
    create_agent,
...
@cli.command()
@click.option(
    "--agent", required=True, help="Agent type to evaluate (e.g., openai, anthropic)"
)
@click.option(
    "--agent-args",
    default="",
    help="Comma-separated key=value pairs for agent configuration",
)
@click.option(
    "--environment",
    required=True,
    help="Environment type for evaluation (e.g., text-to-sql, custom)",
)
@click.option(
    "--environment-args",
    default="",
    help="Comma-separated key=value pairs for environment configuration",
)
@click.option("--context", default=None, help="Context system type (e.g., dbt, custom)")
@click.option(
    "--context-args",
    default="",
    help="Comma-separated key=value pairs for context configuration",
)
@click.option(
    "--interventions", default="", help="Comma-separated list of interventions to apply"
)
@click.option(
    "--config-file",
    default=None,
...
def evaluate(
    ...
    context_instance = (
        create_context_system(config.get("context"))
        if "context" in config and config.get("context")
        else None
    )
...
def _parse_configuration(
    ...
    if interventions:
        config["interventions"] = [
            i.strip() for i in interventions.split(",") if i.strip()
        ]
