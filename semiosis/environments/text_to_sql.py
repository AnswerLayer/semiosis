...
class Spider2TaskGenerator(TaskGenerator):
    ...
    def _load_tasks(self) -> List[SQLTask]:
        ...
        sample_tasks = [
            {
                "query": (
                    "Find the name of all students who are enrolled in the "
            ...
            {
                "query": (
                    "What is the average salary of employees in the "
...
class SQLTaskEvaluator(TaskEvaluator):
    ...
    def evaluate(self, task: Task, response: str) -> EvaluationResult:
        """
        Evaluate a SQL response by executing both the generated and ground truth
        queries.
        ...
        sql_task = (
            ...
            if isinstance(task, SQLTask)
                ...
                database_path=(
                    task.metadata.get("database_path") if task.metadata else None
                ),
            )
        )
        ...
        try:
            ...
            if sql_task.ground_truth:
                ground_truth_result = self._execute_query(
                    sql_task.ground_truth, sql_task.database_path
                )
    ...
    def _execute_query(
        self, sql: str, database_path: Optional[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if execution_time > self.timeout:
                raise TimeoutError(
                    f"Query execution exceeded {self.timeout} second timeout"
                )
